...
export default function PropertyCard({
  ...
  return (
    <Link href={`/properties/${p.slug}`} className="group card-surface flex flex-col overflow-hidden">
      {/* Image */}
      <div className="relative aspect-[4/3] overflow-hidden bg-ink-800">
        ...
        <div className="absolute inset-x-4 top-4 flex flex-wrap items-start gap-1.5">
          {p.exclusive && <span className="chip chip-gold">{t.exclusive}</span>}
          {p.status === "reserved" && (
            <span className="chip bg-ink-950/85 text-bone-50 backdrop-blur-sm">{t.reserved}</span>
          )}
          {p.status === "sold" && (
            <span className="chip bg-ink-950/85 text-bone-50 backdrop-blur-sm">{t.sold}</span>
          )}
          {p.paymentPlan && (
            <span className="chip chip-glass">{fmt(t.plan, { years: p.paymentPlan.years })}</span>
          )}
          <span className="chip chip-glass ms-auto">{p.typeLabel}</span>
        </div>
        ...
        <div className="mt-auto pt-5">
          ...
          <div className="mt-4 flex items-center justify-between gap-3 text-[0.8125rem] text-ink-500">
            <div className="flex items-center gap-4">
              {p.bedrooms > 0 && (
                <span className="inline-flex items-center gap-1.5" title={t.bedrooms}>
                  <BedDouble className="size-4 text-gold-600" strokeWidth={1.5} />
                  {p.bedrooms}
                </span>
              )}
              <span className="inline-flex items-center gap-1.5" title={t.bathrooms}>
                <Bath className="size-4 text-gold-600" strokeWidth={1.5} />
                {p.bathrooms}
              </span>
              <span className="inline-flex items-center gap-1.5" title={t.area}>
                <Maximize2 className="size-4 text-gold-600" strokeWidth={1.5} />
                {formatArea(locale, p.size)}
              </span>
            </div>
            <span dir="ltr" className="font-mono text-[0.6875rem] uppercase tracking-wider text-ink-300">
              {p.ref}
            </span>
          </div>
        </div>
